...
public class MapTile : CMyUnityBase
{
    ...
    //내 아래의 타일의 1~4사분면 중 어느 부분을 밟고 있는지 구한다.
    //(모든 타일 전부 검색하는 로직)
    public int Get_UnderTile_ReturnQuarter(Vector3 v3Pos, ref int _ix, ref int _iy)
    {
        Vector3 v3TempPos;
        float fTileHalfSize = (m_LastTile.transform.localScale.x * 0.5f);

        for (int x = 0; x < m_NumX; x++)
        {
            for (int y = 0; y < m_NumY; y++)
            {
                v3TempPos = m_TileArray[x, y].transform.position;
                if ((v3TempPos.x - fTileHalfSize) <= v3Pos.x
                 && (v3TempPos.x + fTileHalfSize) >= v3Pos.x
                 && (v3TempPos.z - fTileHalfSize) <= v3Pos.z
                 && (v3TempPos.z + fTileHalfSize) >= v3Pos.z)
                {
                    _ix = x;
                    _iy = y;

                    //반환값 int는 1~4 사분면 중 어느 부분 밟고있는지 알려주는 반환값
                    //오른쪽 위가 1, 반시계 방향으로 2 3 4 사분면은 각각 1~4
                    //정 중앙 -> 0
                    //1~2사분면 사이 5, 2~3사분면 사이 6 해서 8까지.
                    //0~8까지 반환값이 있음.

                    //2,3사분면 쪽, 혹은 6
                    if (v3Pos.x < v3TempPos.x)
                    {
                        if(v3Pos.y < v3TempPos.y)
                        {
                            return 3;
                        }
                        else if(v3Pos.y > v3TempPos.y)
                        {
                            return 2;
                        }
                        else
                    ...
                    //0, 5, 7 중 하나, float에서 값이 같기 정말 힘든데..
                    else
                    {
                        if (v3Pos.y < v3TempPos.y)
                        {
